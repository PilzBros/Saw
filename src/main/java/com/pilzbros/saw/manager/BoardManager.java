...
public class BoardManager {
    ...
    public BoardManager(String scoreBoardName, String mainObjectiveName, DisplaySlot slot) {
        this(scoreBoardName, mainObjectiveName, slot, (String[])null);
    }

    public BoardManager(String scoreBoardName, String mainObjectiveName, DisplaySlot slot, String[] lines) {
        this.players = new ArrayList();
        this.boardName = scoreBoardName;
        this.mainObjectiveName = mainObjectiveName;
        this.slot = slot;
        this.scoreBoard = Bukkit.getScoreboardManager().getNewScoreboard();
        this.mainObjective = this.scoreBoard.registerNewObjective("main", BoardManager.ObjectiveCriteria.DUMMY.getName());
        this.mainObjective.setDisplayName(mainObjectiveName);
        this.mainObjective.setDisplaySlot(slot);
        if (lines != null) {
            for(int i = 0; i < lines.length; ++i) {
                this.addObjectiveScore(lines[i], i);
            }
        }
    ...
    public void changeCriteria(BoardManager.ObjectiveCriteria criteria) {
        this.mainObjective.unregister();
        this.mainObjective = null;
        this.mainObjective = this.scoreBoard.registerNewObjective("main", criteria.getName());
    ...
    public void registerObjective(String objective, BoardManager.ObjectiveCriteria criteria) {
    ...
    public void addObjectiveScore(String scoreName, int score) {
        this.mainObjective.getScore(Bukkit.getOfflinePlayer(scoreName)).setScore(score);
    }
    ...
    public void setObjectiveScore(String scoreName, int setScore) {
        this.getObjectiveScore(scoreName).setScore(setScore);
    }

    public void setObjectiveScore(Objective objective, String scoreName, int setScore) {
        this.getObjectiveScore(objective, scoreName).setScore(setScore);
    }
    ...
    public static enum ObjectiveCriteria {
        ...
        public String getStatWithID(int id) {
            return this.name + "." + id;
        }

        public String getStatWithID(String stat, int id) {
            return "stat." + stat + "." + id;
        }
    }
}
